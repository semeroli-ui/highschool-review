...
if not st.session_state.logged_in:
    ...
    if mode == "智脑看板":
        st.markdown("## 📊 同步状态")
        stats = db.document(f"{get_public_path()}/stats/global").get().to_dict() or {}
        ...
        for item in data:
            ...
            with st.expander(f"{icon} {item['title']}"):
                ...
                if c3.checkbox("掌握", key=f"m_{m_key}", value=is_m):
                    if not is_m: update_cloud_node(u, subject_id, item['title'], m=True); st.rerun()
                elif is_m: update_cloud_node(u, subject_id, item['title'], m=False); st.rerun()
